...
export async function getJurnalDetail(id: string, isPublicOnly = true) {
  ...
  if (isPublicOnly) {
    conditions.push(eq(jurnal.is_published, true))
  }
  const results = await db.select().from(jurnal).where(and(...conditions)).limit(1)
  return results[0] || null
}

export async function getJurnalDetailBySourceId(sourceId: string, isPublicOnly = true) {
  let conditions = [eq(jurnal.source_id, sourceId)]
  if (isPublicOnly) {
    conditions.push(eq(jurnal.is_published, true))
  }
  const results = await db.select().from(jurnal).where(and(...conditions)).limit(1)
